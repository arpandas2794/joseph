...
const getPlatformConfig = (type: string) => {
  switch (type) {
    case 'youtube': return { icon: PlayCircle, color: 'text-red-500', bg: 'bg-red-500/10', label: 'YouTube' };
    case 'instagram':
    case 'instagram_carousel': return { icon: Camera, color: 'text-pink-500', bg: 'bg-pink-500/10', label: 'Instagram' };
    case 'tiktok': return { icon: Music2, color: 'text-cyan-400', bg: 'bg-cyan-400/10', label: 'TikTok' };
    case 'google_drive': return { icon: HardDrive, color: 'text-blue-500', bg: 'bg-blue-500/10', label: 'Google Drive' };
    case 'loom': return { icon: Video, color: 'text-[#625DF5]', bg: 'bg-[#625DF5]/10', label: 'Loom' };
    case 'document': return { icon: FileText, color: 'text-indigo-400', bg: 'bg-indigo-400/10', label: 'Document' };
    case 'website': return { icon: Globe, color: 'text-emerald-400', bg: 'bg-emerald-400/10', label: 'Website' };
    case 'voice': return { icon: Mic, color: 'text-amber-400', bg: 'bg-amber-400/10', label: 'Voice Note' };
    case 'sticky': return { icon: StickyNote, color: 'text-yellow-400', bg: 'bg-yellow-400/10', label: 'Note' };
    default: return { icon: FileText, color: 'text-gray-400', bg: 'bg-gray-400/10', label: 'Asset' };
  }
};

export default function MobileAssetCard({ node, onOpenActions, onTap, onDelete, onConnectToChat }: MobileAssetCardProps) {
