...
export default function InstagramNode({ id, data, selected }: { id: string, data: any, selected?: boolean }) {
  const { id: workspaceId } = useParams<{ id: string }>();
  const removeNode = useCanvasStore((state) => state.removeNode);
  const [showTranscript, setShowTranscript] = useState(false);
  const [customTitle, setCustomTitle] = useState(data.customTitle || 'Instagram');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error("Failed to delete Instagram node:", err);
    }
  };
  ...
  return (
    <>
      ...
      />
      <div
        className={`w-full h-full bg-[#18181b] text-white rounded-xl shadow-2xl border flex flex-col overflow-hidden transition-all duration-200 ${selected ? 'border-pink-500 shadow-pink-500/20' : 'border-white/10'
          }`}
      >
        {/* Header */}
        <div className="h-12 flex-shrink-0 bg-white/5 border-b border-white/5 flex items-center px-3 justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-1 rounded transition-colors text-xs flex items-center gap-1 font-medium ${showTranscript ? 'bg-pink-500/20 text-pink-400' : 'text-gray-400 hover:text-white bg-white/5'}`}
        ...
        <div className="flex-1 w-full bg-black relative min-h-0">
          {showTranscript ? (
            ...
            >
              <h4 className="font-semibold text-gray-200 mb-2 font-mono">Instagram Transcript (AI Extracted):</h4>
            ...
            thumbnail ? (
              <div className="w-full h-full relative group">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl text-sm font-semibold transition-all shadow-lg hover:scale-105 active:scale-95 flex items-center gap-2"
                  >
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-gray-500 text-sm p-4 text-center">
                <Camera className="w-8 h-8 text-pink-500/50" />
