...
export default function VoiceNode({ id, data, selected }: { id: string; data: any; selected?: boolean }) {
  const { id: workspaceId } = useParams<{ id: string }>();
  const removeNode = useCanvasStore((state) => state.removeNode);

  const [customTitle, setCustomTitle] = useState(data.customTitle || 'Voice Note');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error('Failed to delete voice node:', err);
    }
  };
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className={`p-1 rounded transition-colors text-xs flex items-center gap-1 font-medium ${
                showTranscript ? 'bg-purple-500/20 text-purple-400' : 'text-gray-400 hover:text-white bg-white/5'
              }`}
          ...
          {showTranscript ? (
            ...
            >
              <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-white/5 flex-shrink-0">
                <AlignLeft className="w-3.5 h-3.5 text-purple-400 flex-shrink-0" />
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-gray-500 gap-3">
              ...
              <p className="text-xs text-center font-medium">Voice Recording</p>
            </div>
          )}
        </div>
